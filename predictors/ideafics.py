...
class Qwen2VLPredictor(Predictor):
    ...
    def generate_prediction(self, messages, image_link=None):
        ...
        if image_link:
            image = f"/mnt/workspace/Projects/CS6223/data/images/v3/{image_link}"
        # print(image)
            inputs = self.processor(text=prompt, images=[load_image(image)], return_tensors="pt")
        else:
...
if __name__ == '__main__':
    ...
    import glob
    import torch
    with open('configs/config-pred-idefics-llama3.yaml', 'r') as file:
