...
class Generator:
    ...
    def __init__(self,
        ...
        self.needle = needle
        self.image_needle = image_needle
    ...
    async def generate_context(self, context_length, depth_percent):
        ...
        prefix, suffix = self.get_partial_context(context, depth_percent, context_length)

        needle = self.needle if self.image_link is None else self.image_needle
        # prefix, needle
        build_prompt = {
            "prefix": prefix,
            "needle": needle,
    ...
    def get_partial_context(self, context, depth_percent, context_length):
        ...
        if depth_percent == 100:
            ...
            prefix = tokens_context[:insertion_point]
            suffix = tokens_context[insertion_point:]
        
        # new_context = self.decode_tokens(tokens_new_context)
        prefix = self.decode_tokens(prefix)
        suffix = self.decode_tokens(suffix)
...
if __name__ == '__main__':
    with open('configs/config-generate.yaml', 'r') as file:
        config = yaml.load(file, Loader=yaml.FullLoader)

    ht = Generator(
        needle=config['prompt']['needle'],
        haystack_dir=config['prompt']['haystack_dir'],
        image_link=config['prompt']['image'],
        image_needle=config['prompt']['image_needle'],
